...
#       2) Schools fully remote                                                    # 
#       3) Schools open - universal masking                                        # 
#       4) Schools open - masking and cohort approach                              # 
#       5) Schools open - masking and weekly testing                               # 
#####################################################################################  
...
outcomes <- foreach(reps = 1:1000, .packages = "dplyr") %dopar% {
  ...
  VE <- got_v*VE_any # a vector to be multiplied by the FOI, will be 0 if not vaccinated, VE_any else
  
  #Define conditional probabilities for clinical outcomes 
  #rho: prob infection is clinical given age and vaccine status
  ...
  #update P(clin|age,vac) for those who are vaccinated
  ...
  CLOSED_contact_list[[6]] <- gen_COMmat(Obs.comm_feb, Age_Vect2, synth_pop_df) #Community, using data from survey
  ...
  StatesPhase_CLOSED <- SEIRQ_func(synth_pop_df, 
                                    ...
                                    propComplyHH = 0.25,
                                    redContacts = 0.75,
                                    mask_ef_teacher = 0, #0 when nobody is masking
                                    mask_ef_elem = 0, #0 when nobody is masking
                                    mask_ef_middle = 0, #0 when nobody is masking
                                    mask_ef_HS = 0) #0 when nobody is masking
  ...
  ################## Sim 2: Opening schools fully!!! ############################
  OPEN_contact_list <- contacts_orig_list #se-set to schools being open
  
  OPEN_contact_list[[2]] <- MidEssentialWorkMatsp #Work
  OPEN_contact_list[[6]] <- gen_COMmat(Obs.comm_feb, Age_Vect2, synth_pop_df) #Community, using survey rates
  ...
  StatesPhase_OPEN <- SEIRQ_func(synth_pop_df, 
                                  ...
                                  propComplyHH = 0.25,
                                  redContacts = 0.75,
                                  mask_ef_teacher = 0, #no masks
                                  mask_ef_elem = 0, #no masks
                                  mask_ef_middle = 0, #no masks
                                  mask_ef_HS = 0) #no masks
  ...
  StatesPhase_MASK <- SEIRQ_func(synth_pop_df, 
                                  ...
                                  propComplyHH = 0.25,
                                  redContacts = 0.75,
                                  mask_ef_teacher = 0.5, #wearing masks, 50% effectiveness
                                  mask_ef_elem = 0.15, #wearing masks, 15% effectiveness
                                  mask_ef_middle = 0.25, #wearing masks, 25% effectiveness
                                  mask_ef_HS = 0.35) #wearing masks, 35% effectiveness
  ...
  S1_contact_list[[2]] <- MidEssentialWorkMatsp #Work
  S1_contact_list[[3]] <- 0.25*S1_contact_list[[3]] #reduce non-class contacts by 75%
  S1_contact_list[[4]] <- 0.25*S1_contact_list[[4]] #reduce non-class contacts by 75%
  S1_contact_list[[5]] <- (5/7 - 0.25*1/7 - 0.25*.2/7)/(5/7 - 1/7 - 0.2/7)*S1_contact_list[[5]]
  S1_contact_list[[6]] <- gen_COMmat(Obs.comm_feb, Age_Vect2, synth_pop_df) #community, using survey rates
  
  #Call SEIR function
  StatesPhase_MASK_COHORT <- SEIRQ_func(synth_pop_df, 
                                start_day = 1, #Aug 9
                                end_day = 128, #Dec 17
  ...
  state_full.S1 <- StatesPhase_MASK_COHORT[["state_full"]]
  ...
  ################### Sim 5: Test teachers and students weekly ############################
  
  #Call SEIR function that permits testing
  StatesPhase_TB7 <- SEIRtest_func(synth_pop_df, 
                                    start_day = 1, #Aug 15
                                    end_day = 128, #Dec 20 
                                    test_day = 7, #test every 7 days
                                    test_students = 1, #do test students
                                    contacts_orig_list = OPEN_contact_list, 
                                    alpha,
                                    beta = wait_times[["beta"]],
                                    time_state1 = wait_times[["time_state1"]], time_next_state1 = wait_times[["time_next_state1"]],
  ...
  state_full.TB7 <- StatesPhase_TB7[["state_full"]]
  
  #extract summary indicators
  outcomeTB7 <- modelled_outcomes(state_full.TB7, synth_pop_df)
  ...
  #Don't need to save synthetic pop and fates bc already have that saved.
