...
gen_COMmat <- function(matrix_name, Age_Vect2, synth_pop_df #summary matrix of total # of age-structed daily contacts
){
...
SEIRQ_func <- function(synth_pop_df, #synthetic population
                      ...
                      who_mask = 1 # who will mask (default = "All", otherwise provide vector)
){
  ...
  for (day in start_day:end_day){
    ...
    if (length(who_mask) == 1) { #this is the default condition - everyone masks
      who_mask <- rep(1,N)
    }
    mask_ef[!is.na(synth_pop_df$School) & synth_pop_df$Age>=18 & who_mask == 1] <- 1-mask_ef_teacher #teachers
    mask_ef[!is.na(synth_pop_df$School) & synth_pop_df$Age>=11 & synth_pop_df$Age < 13 & who_mask == 1] <- 1-mask_ef_middle #middle school students
    mask_ef[!is.na(synth_pop_df$School) & synth_pop_df$Age >= 13 & synth_pop_df$Age < 18 & who_mask == 1] <- 1-mask_ef_HS #high school students
...
modelled_outcomes <- function(state_full,#output of SEIR function - dataframe of states per agent per day
  ...
  school_start_state <- state_full[,1] #initial state
